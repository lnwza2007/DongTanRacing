...
// ── Invalidation flag (forces full redraw after menu exit) ──
static bool _db_dirty = false;
static void db_invalidate() { _db_dirty = true; }
...
// ============================================================
// Returns true if a full redraw was performed (caller should reset its own statics)
static bool db_update(TFT_eSPI &tft,
                       int speed,
                       int rampUp, int rampDown, int speedLim,
                       int mode,
                       float yprRad[3]) {
  ...
  if (_db_dirty) {
    prevSpeed = -1; prevRampUp = -1; prevRampDown = -1;
    prevSpeedLim = -1; prevMode = -1; prevSec = 0;
    _db_dirty = false;
    didFullRedraw = true;
  }
  ...
  if (paramsChanged || didFullRedraw) {
...
static void db_drawSetParam(TFT_eSPI &tft,
  ...
  if (value == prevVal && !_db_dirty) return;
  prevVal = value;
